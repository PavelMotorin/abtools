...
class BootstrapAB:
    ...
    def fit(self, data:pd.DataFrame, response_var:str, variant_key:str, n_samples=2000, n_jobs=None) -> None:
        """
        Runs A/B test for given data.
        
        Use given response_var as target for this test and variant_key to select variants from data.
        """
        variants = data[variant_key].unique()
        
        for variant in variants:
            b_dist = BootstrapDistribution(variant, self.bootstrap_func, n_samples, n_jobs)
            arr = data.loc[data[variant_key] == variant, response_var]
    ...
    def visualize(self, *args, **kwargs):
        ...
        source.columns = [str(x) for x in source.columns]
